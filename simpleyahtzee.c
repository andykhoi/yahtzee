...
void scoreRound(int dice [NUM_DICE], int categoryScores[NUM_FACES]) {
  ...
  // 4) Print the current total score
	...
	while (categoryNum == 0) {
		...
		scanf("%d", &categoryNum);
        	flushStdin();
		if (categoryNum > 0 && categoryNum < 7){ //this branch checks if user input is [1,6] 
			if (categoryScores[categoryNum-1] == UNSCORED_CATEGORY) { //this branch checks if the user chosen category is unscored
				for (int i = 0; i < NUM_DICE; ++i) { //this loop searches for dice value equal to user chosen category
					if (dice[i] == categoryNum) {
						score = score + categoryNum;
					}
				categoryScores[categoryNum-1] = score;
				}
			}
			else {//prints an error if user chosen category is already scored
				printf("Error: invalid command. Enter 1-6 to save to an unused category\n");
				categoryNum = 0;
			}
		}
		else { //this branch prints an error is user chosen category is not [1,6]
			printf("Error: invalid command. Enter 1-6 to save to an unused category\n");
			categoryNum = 0;	
		}
	}
	for(int j = 0; j < NUM_FACES; ++j) {
		if (categoryScores[j] == UNSCORED_CATEGORY) {
			;
		}
		else{
			totalScore = totalScore + categoryScores[j];
		}
	}
...
void printTotalScore(int categoryScores[NUM_FACES]) {
  ...
  // Category 6 score: 12
  // Total Score: 27
 // printf("Category 6: %d", categoryScores[5]);
	for(int i = 1; i < NUM_FACES + 1; ++i) {
		printf("Category %d score: ", i);
		if (categoryScores[i-1] == UNSCORED_CATEGORY) {
			printf("not scored\n");
		}
		else {
			printf("%d\n", categoryScores[i-1]);
